...
{"key":"stick_hide_from_game","type":"bool","default":false,"help":"IMPORTANT WHEN A STICK MOVES THE CURSOR OR SCROLLS. Hides that stick from the game, so pointing does not also steer. The OTHER stick is untouched and can still walk. There are two sticks, so this acts on whichever one the settings below have named -- with none pointing or scrolling, there is nothing for it to hide."},
...
{"key":"touchpad_hide_from_game","type":"bool","default":false,"help":"IMPORTANT WHEN THE TOUCHPAD MOVES THE CURSOR OR SCROLLS. Hides the touchpad from the game entirely -- finger positions and the touchpad click. Without it every finger movement reaches the game as well as the cursor. Turn it on with any touchpad mouse setting below."},
...
{"key":"gyro_hide_from_game","type":"bool","default":false,"help":"IMPORTANT WHEN THE GYRO MOVES THE CURSOR. Hides the gyro from the game entirely. Without it you are aiming twice at once: the cursor moves AND the game camera turns, because the game still receives the motion the cursor is using. Turn it on with any gyro-to-mouse setting below. It also works alone, for a game that reads motion you never asked it to."},
