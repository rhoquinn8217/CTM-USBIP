...
{"key":"trigger_r2_snap_force","type":"int","min":1,"max":7,"default":3,"help":"How hard the \"snap\" effect tries to push R2 back to rest. Only used by \"snap\". If raising this makes the break HARDER rather than the return stronger, we have the wrong byte and the shape should be reported as such."},
{"key":"trigger_l2_snap_force","type":"int","min":1,"max":7,"default":3,"help":"How hard the \"snap\" effect tries to push L2 back to rest."},
...
{"key":"trigger_r2_click","type":"string","name":"R2 press sends","default":"","help":"What R2 sends once it passes the point below, using the same names as a button remap: MouseLeft, MouseRight, MouseMiddle, Enter, Space and so on. Blank is off. Starting to pull freezes the cursor so the press lands still, and a full release hands it back. Pair it with an R2 effect at the same point so the finger can feel where it fires."},
