...
{"key":"right_trigger_bind","type":"string","name":"(R2) | (RT) — RIGHT trigger","default":"","help":"What the RIGHT trigger sends, using the same names as a button remap. ⭐ Setting this also turns on the cursor gesture for that trigger: starting to pull FREEZES the cursor, passing the point sends, and only letting the trigger all the way home hands the cursor back. It REPLACES the (R2) remap, which is ignored while this is set. What it sends once it passes its point, using the same names as a button remap: MouseLeft, MouseRight, MouseMiddle, Enter, Space and so on. Blank is off. Starting to pull freezes the cursor so the press lands still, and a full release hands it back."},
{"key":"right_trigger_bind_at","type":"int","min":10,"max":100,"default":80,"name":"RIGHT trigger fires at","help":"How far the RIGHT trigger must travel before it sends, as a percent of the pull. IGNORED when the effect is \"click\" or \"wall\": those report their own point and the press fires there instead, which is why it lands exactly where your finger feels it. Used by \"notch\" and \"snap\" and by a trigger with no effect. Deeper is steadier, because the finger has settled by the time it fires."},
{"key":"right_trigger_effect","type":"choice","choices":["","off","click","wall","notch","snap"],"default":"","name":"RIGHT trigger feel","help":"A resistance effect on the RIGHT trigger. \"click\" is a break at the point below, so the finger can feel where it fires. \"wall\" is steady resistance from that point down. \"notch\" is both: a light wall the whole way with one firm detent at the point. \"snap\" is a break that also pushes back toward rest, and is on trial. Blank leaves the trigger alone; \"off\" clears an effect this listener set. Wired controllers only."},
{"key":"right_trigger_effect_at","type":"int","min":0,"max":100,"default":50,"name":"RIGHT trigger feel sits at","help":"Where in the pull the RIGHT effect sits, as a percent. Leave it blank to follow the point the trigger fires at, which is what you want unless you are deliberately marking somewhere else. A \"click\" or \"snap\" break can only land between 30 and 90 percent."},
{"key":"right_trigger_effect_strength","type":"int","min":1,"max":7,"default":5,"name":"RIGHT trigger feel strength","help":"How hard the RIGHT effect pushes back, 1 lightest and 7 firmest. A \"notch\" needs 5 or more: below that its detent and its wall are too close to tell apart and it feels like a plain wall. Every value is a real force; none of them quietly means none."},
{"key":"right_trigger_snap_force","type":"int","min":1,"max":7,"default":3,"name":"RIGHT trigger snap force","help":"Only used by \"snap\". How hard it tries to push the RIGHT trigger back to rest once you ease off."},
{"key":"left_trigger_bind","type":"string","name":"(L2) | (LT) — LEFT trigger","default":"","help":"What the LEFT trigger sends, using the same names as a button remap. ⭐ Setting this also turns on the cursor gesture for that trigger: starting to pull FREEZES the cursor, passing the point sends, and only letting the trigger all the way home hands the cursor back. It REPLACES the (L2) remap, which is ignored while this is set. What it sends once it passes its point. The same names and the same behaviour as the RIGHT trigger above."},
{"key":"left_trigger_bind_at","type":"int","min":10,"max":100,"default":80,"name":"LEFT trigger fires at","help":"How far the LEFT trigger must travel before it sends, as a percent of the pull. IGNORED when the effect is \"click\" or \"wall\": those report their own point and the press fires there instead, which is why it lands exactly where your finger feels it. Used by \"notch\" and \"snap\" and by a trigger with no effect."},
{"key":"left_trigger_effect","type":"choice","choices":["","off","click","wall","notch","snap"],"default":"","name":"LEFT trigger feel","help":"A resistance effect on the LEFT trigger. The same shapes as the RIGHT trigger above."},
{"key":"left_trigger_effect_at","type":"int","min":0,"max":100,"default":50,"name":"LEFT trigger feel sits at","help":"Where in the pull the LEFT effect sits, as a percent. Blank follows the point the trigger fires at."},
{"key":"left_trigger_effect_strength","type":"int","min":1,"max":7,"default":5,"name":"LEFT trigger feel strength","help":"How hard the LEFT effect pushes back, 1 lightest and 7 firmest. A \"notch\" needs 5 or more: below that its detent and its wall are too close to tell apart and it feels like a plain wall."},
{"key":"left_trigger_snap_force","type":"int","min":1,"max":7,"default":3,"name":"LEFT trigger snap force","help":"Only used by \"snap\". How hard it tries to push the LEFT trigger back to rest."},
{"key":"trigger_freeze_at","type":"int","min":4,"max":50,"default":15,"name":"Both triggers: cursor freezes at","help":"How far EITHER trigger must move before the cursor freezes, as a percent of the pull. It wants to be small so the cursor is still well before the press, but NOT close to where the trigger rests: a resting trigger was measured wandering up to 7 percent with an effect set, because the effect holds it off its stop, and a threshold near that is read as a finger. One number for both, since it describes the hand rather than the trigger."},
{"key":"trigger_drag_after_ms","type":"int","min":0,"max":2000,"default":600,"name":"Both triggers: hold becomes a drag after","help":"How long a trigger press is held before it becomes a drag, in milliseconds. This is also the double-press window: hold for less and a second press keeps the cursor frozen throughout, hold for longer and the cursor comes back with the button still down. A deliberate press measured 538ms, so anything near 200 turns ordinary presses into drags. 0 turns dragging off."},
{"key":"trigger_probe","type":"bool","default":false,"name":"Both triggers: log the raw report","help":"Diagnostic, for both trigger logs. Logs every press, freeze and drag transition, and any input-report byte that changes apart from the ones that always do. Noisy; leave it off unless something needs explaining."},
{"key":"gyro_to_mouse_gate","type":"choice","choices":["","always","L2","R2","L1","R1","touchpad","!touchpad","touchpad_click","trigger","PS"],"default":"","help":"What must be held for gyro to move the mouse. Blank is off. \"trigger\" is the opposite of the others: the cursor moves EXCEPT while you are working a trigger, so a click lands on a cursor that is already still. It comes back when the trigger goes fully home, or once a held click has become a drag."},
