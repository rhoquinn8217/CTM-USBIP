...
{"key":"trigger_r2_fires_mouse","type":"bool","default":false,"help":"Fire by clicking the virtual mouse when R2 is pressed, instead of relying on the trigger effect. For games that read the shot from the weapon-mode break -- replace that effect and the gun stops firing, so the shot has to come from somewhere else."},
{"key":"trigger_r2_fire_threshold","type":"int","min":1,"max":255,"default":32,"help":"How far R2 must travel to count as firing, out of 255. Early by default: with the weapon break replaced there is no longer a tactile point telling you where the trigger fires."},
{"key":"trigger_r2_fire_hz","type":"int","min":0,"max":100,"default":20,"help":"Clicks per second while R2 is held. ⭐ 0 HOLDS the button instead of clicking -- worth trying, since our virtual mouse keeps sending reports while held where a real one goes silent. Otherwise the game paces its own fire rate, so this only has to stay above it."},
{"key":"trigger_r2_match","type":"string","default":"","help":"Hex prefix of the R2 trigger block to replace, e.g. '25 0c 00'. Blank leaves every effect alone. Turn on trigger_watch to see what a game sends."},
{"key":"trigger_r2_replace","type":"string","default":"","help":"Hex block to send instead, starting with the mode byte: 0x26 vibration or 0x27 machine both oscillate, 0x25 weapon does not. Raw bytes on purpose -- the parameter layout is unverified, so it is found by feel."},
{"key":"trigger_watch","type":"bool","default":false,"help":"Logs the adaptive trigger bytes whenever they CHANGE, so a game's effect can be identified before replacing it. Only on change -- a game drives the triggers on every report, so logging all of them buries the moment that matters."},
