...
function Save-Now {
    try {
        ...
        $lines = Write-Setting $lines 'ds5' 'audio_output'       $output.SelectedItem
        $lines = Write-Setting $lines 'ds5' 'speaker_volume'     "$($speaker.Value)"
        $lines = Write-Setting $lines 'ds5' 'headset_volume'     "$($headset.Value)"
        $lines = Write-Setting $lines 'ds5' 'audio_gain'         "$($gain.Value)"
        $lines = Write-Setting $lines 'ds5' 'master_rumble_gain' "$($rumble.Value)"
        $lines = Write-Setting $lines 'ds5' 'force_echo_cancel'  $(if ($echo.Checked) { 'true' } else { 'false' })
