...
#
# ⓘ The report descriptor declares LED outputs (caps lock and friends) because a
# boot keyboard does. With no OUT endpoint the host sends those over EP0 as
# SET_REPORT, which is normal and needs nothing from us -- we simply ignore them.
#
...
#             wReportLength 0x003F (63 bytes -- see the report descriptor).
...
# and cannot lose one.
bytes = 0x09 0x02 0x22 0x00 0x01 0x01 0x00 0xA0 0x32 0x09 0x04 0x00 0x00 0x01 0x03 0x01 0x01 0x00 0x09 0x21 0x11 0x01 0x00 0x01 0x22 0x3F 0x00 0x07 0x05 0x81 0x03 0x08 0x00 0x01

[hid_report_descriptor]
# 63-byte boot-keyboard report descriptor, from the USB HID spec's own example.
# Taken verbatim rather than derived, so it cannot be subtly wrong.
#
